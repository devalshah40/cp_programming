/*
Q6. Is Dictionary?
Unsolved
character backgroundcharacter
Stuck somewhere?
Ask for help from a TA and get it resolved.
Get help from TA.
Problem Description
Surprisingly, in an alien language, they also use English lowercase letters, but possibly in a different order. The order of the alphabet is some permutation of lowercase letters.

Given an array of words A of size N written in the alien language, and the order of the alphabet denoted by string B of size 26, return 1 if and only if the given words are sorted lexicographically in this alien language else, return 0.



Problem Constraints
1 <= N, length of each word <= 105

Sum of the length of all words <= 2 * 106



Input Format
The first argument is a string array A of size N.

The second argument is a string B of size 26, denoting the order.



Output Format
Return 1 if and only if the given words are sorted lexicographically in this alien language else, return 0.



Example Input
Input 1:

 A = ["hello", "scaler", "interviewbit"]
 B = "adhbcfegskjlponmirqtxwuvzy"
Input 2:

 A = ["fine", "none", "no"]
 B = "qwertyuiopasdfghjklzxcvbnm"


Example Output
Output 1:

 1
Output 2:

 0


Example Explanation
Explanation 1:

 The order shown in string B is: h < s < i for the given words. So return 1.
Explanation 2:

 "none" should be present after "no". Return 0.



See Expected Output
Your input
2 he hi adhbcfegskjlponmirqtxwuvzy
Output
1
*/
// let A = ['he', 'hi'];
// let B = 'hei';
let A = ['hello', 'scaler', 'interviewbit'];
let B = 'adhbcfegskjlponmirqtxwuvzy';
function isDictionary(A, B) {
  let map = new Map();
  for (let i = 0; i < B.length; i++) {
    const char = B[i];
    map.set(char, i);
  }
  map.set(-1, -1);

  for (let i = 0, j = 1; j < A.length; i++, j++) {
    let previousStr = A[i];
    let currentStr = A[j];

    let k = 0;

    let isSuccessful = false;
    while (previousStr[k] || currentStr[k]) {
      let previousChar = previousStr[k] ? previousStr[k] : -1;
      let currentChar = currentStr[k] ? currentStr[k] : -1;
      let value1 = map.get(previousChar);
      let value2 = map.get(currentChar);

      if (value1 < value2) {
        isSuccessful = true;
        break;
      } else if (value1 > value2) {
        return 0;
      }
      k++;
    }
  }

  return 1;
}
let ans = isDictionary(A, B);
console.log(ans);

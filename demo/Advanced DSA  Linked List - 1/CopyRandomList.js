...
function copyRandomList(head) {
  ...
  while (h1) {
    temp = h1;
    if (temp.random) {
      const newNode = new Node(temp.random.data);
...
function copyList(head) {
  ...
  const map = new Map();
  let temp = null;
  let h2 = new RandomListNode();
  let newTail = h2;
  while (h1) {
    temp = h1;
    h1 = h1.next;
    const newNode = new RandomListNode(temp.data);
  ...
  while (h1) {
    if (h1.random) {
      const randomNode = map.get(h1.random);
      newTail.random = randomNode;
    }
    h1 = h1.next;
    newTail = newTail.next;
  }
...
function printAll(head) {
  ...
  while (temp !== null) {
    ...
    if (temp.random) console.log(temp.random.data);
    temp = temp.next;
  }
}
